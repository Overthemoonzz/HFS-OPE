...
#     """
#     def __init__(self, input_feature_size, expert_hidden_size, expert_output_size, dropout: float = 0.2):
#         super().__init__()
...
class CGC(nn.Module):
    ...
    def __init__(
        ...
        self.experts_shared = nn.ModuleList([
            Expert(self.input_size, self.experts_out)
            for _ in range(self.num_shared_experts)
        ])
...
class PLE(nn.Module):
    ...
    def __init__(
        ...
        if self.task_name == 'kuairand_1k':
            if user_feature_dict is None or item_feature_dict is None or context_feature_dict is None:
                raise Exception("user_feature_dict and item_feature_dict and context_feature_dict must be not None")
            if not isinstance(user_feature_dict, dict) or not isinstance(item_feature_dict, dict) or not isinstance(context_feature_dict, dict):
                raise Exception("user_feature_dict and item_feature_dict and context_feature_dict must be dict")
        ...
        hidden_size = (
            emb_dim * (user_cate_feature_nums + item_cate_feature_nums + context_cate_feature_nums)
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        """
        assert x.size(1) == len(self.user_feature_dict) + len(self.item_feature_dict) + len(self.context_feature_dict)
        ...
        context_embed = torch.cat(context_embed_list, dim=1)
        hidden = torch.cat([user_embed, item_embed, context_embed], dim=1).float()
