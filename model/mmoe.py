...
class MMOE(nn.Module):
    ...
    def __init__(self, user_feature_dict, item_feature_dict, context_feature_dict, task_name=None, emb_dim=128, n_expert=2, mmoe_hidden_dim=128,
                 hidden_dim=[128, 128], dropouts=[0.2, 0.2], output_size=1, expert_activation=F.relu, num_task=2, device=None):
        """
        ...
        :param context_feature_dict: context_feature_dict include: {feature_name: (feature_unique_num, feature_index)}
        ...
        if self.task_name == 'kuairand':
            if user_feature_dict is None or item_feature_dict is None or context_feature_dict is None:
                raise Exception("input parameter user_feature_dict and item_feature_dict must be not None")
            if isinstance(user_feature_dict, dict) is False or isinstance(item_feature_dict, dict) is False or isinstance(context_feature_dict, dict) is False:
        ...
        # embedding初始化
        # user_feature_dict = {feature:(n_qnique, idx)......}
        user_cate_feature_nums, item_cate_feature_nums, context_cate_feature_nums = 0, 0, 0
        ...
        for context_cate, num in self.context_feature_dict.items():
            if num[0] > 1:
                ...
                setattr(self, context_cate, nn.Embedding(num[0], emb_dim))

        # user embedding + item embedding
        hidden_size = emb_dim * (user_cate_feature_nums + item_cate_feature_nums + context_cate_feature_nums) + \
        ...
        # gates
        self.gates = nn.ParameterList([nn.Parameter(torch.empty(hidden_size, n_expert), requires_grad=True) for _ in range(num_task)])
        for gate in self.gates:
            gate.data.normal_(0, 1)
        self.gates_bias = nn.ParameterList([nn.Parameter(torch.empty(n_expert), requires_grad=True) for _ in range(num_task)])
    ...
    def forward(self, x):
        assert x.size()[1] == len(self.item_feature_dict) + len(self.user_feature_dict) + len(self.context_feature_dict)
        # embedding
        user_embed_list, item_embed_list, context_embed_list = list(), list(), list()
        ...
        for context_feature, num in self.context_feature_dict.items():
            if num[0] > 1:
                context_embed_list.append(getattr(self, context_feature)(x[:, num[1]].long()))
            else:
                context_embed_list.append(x[:, num[1]].unsqueeze(1))

        # embedding 融合
        user_embed = torch.cat(user_embed_list, axis=1)
        item_embed = torch.cat(item_embed_list, axis=1)
        context_embed = torch.cat(context_embed_list, axis=1)

        # hidden layer
        hidden = torch.cat([user_embed, item_embed, context_embed], axis=1).float()  # batch * hidden_size
        ...
        for idx, gate in enumerate(self.gates):
            ...
            if self.gates_bias:
                self.gates_bias[idx] = self.gates_bias[idx]
        ...
        return task_outputs
    # task_outputs[i].shape = [batch_size, 1]
