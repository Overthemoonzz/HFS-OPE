...
class MMOE(nn.Module):
    ...
    def __init__(self, user_feature_dict, item_feature_dict, task_name=None, emb_dim=128, n_expert=2, mmoe_hidden_dim=128,
        ...
        # use user_feature_dict to store all feature for 'census_income' dataset
        elif self.task_name == 'census_income':
            item_feature_dict = {}
            if user_feature_dict is None:
                raise Exception("input parameter user_feature_dict and item_feature_dict must be not None")
            if isinstance(user_feature_dict, dict) is False:
                raise Exception("input parameter user_feature_dict and item_feature_dict must be dict")
    ...
    def forward(self, x):
        ...
        if len(item_embed_list) > 0: item_embed = torch.cat(item_embed_list, axis=1)
        ...
        return task_outputs
    # task_outputs[i].shape = [batch_size, output_size]
