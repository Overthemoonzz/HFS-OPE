...
# census-income
def censusincome_data(path, args):
    """
    ...
    train_df = pd.read_csv(path+'/census-income.data.gz', delimiter=',', header=None,
                           index_col=None, names=column_names)
    test_df = pd.read_csv(path+'/census-income.test.gz', delimiter=',', header=None,
                          index_col=None, names=column_names)
...
# kuairand_pure
def kuairand_pure_data(path, args):
    if not path:
        return
    label_columns = ['is_click', 'is_like', 'is_follow', 'is_comment', 'is_forward']
    if args.mtl_task_num == 2:
        label_columns = ['is_click', 'is_like']
    ...
    usecols = categorical_columns + label_columns
