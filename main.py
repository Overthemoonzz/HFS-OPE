...
def get_data(args):
    ...
    if name == 'tenrec':
        ...
        train_data, val_data, test_data, user_feature_dict, item_feature_dict = censusincome_data(path, args)
    elif name == 'kuairand_pure':
        train_data, val_data, test_data, user_feature_dict, item_feature_dict = kuairand_pure_data(path, args)
    #dataloader
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', default='', help='only using it when training hfsope')
    ...
    if args.model_name == 'esmm':
        model = ESMM(user_feature_dict, item_feature_dict, task_name, emb_dim=args.embedding_size, num_task=args.mtl_task_num)
    elif args.model_name == 'mmoe':
        model = MMOE(user_feature_dict, item_feature_dict, task_name, emb_dim=args.embedding_size, num_task=args.mtl_task_num)
    elif args.model_name == 'ple':
        model = PLE(user_feature_dict, item_feature_dict, task_name, emb_dim=args.embedding_size, num_task=args.mtl_task_num)
